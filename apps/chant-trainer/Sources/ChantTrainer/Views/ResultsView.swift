...
struct ResultsView: View {
    ...
    private var trendData: [(index: Int, accuracy: Double)]? {
        guard let recent = progressModel.performanceHistory[exercise.id],
              recent.count >= 3 else { return nil }
    ...
    var body: some View {
        ZStack {
            ...
            if showConfetti {
                ConfettiCanvas(time: confettiTime)
                    .ignoresSafeArea()
            ...
            ScrollView {
                VStack(spacing: CT.largeSpacing) {
                    ...
                    HStack(spacing: CT.spacing) {
                        ForEach(0..<3) { i in
                            Image(systemName: i < visibleStars ? "star.fill" : "star")
                                .font(.largeTitle)
                                .foregroundStyle(i < visibleStars ? .yellow : Color.primary.opacity(0.3))
                                .scaleEffect(i < visibleStars ? 1.0 : 0.6)
                                .opacity(i < visibleStars ? 1.0 : 0.4)
                    ...
                    if let trend = trendData {
                        VStack(alignment: .leading, spacing: CT.smallSpacing) {
                            ...
                            Text(improving ? "Improving!" : "Keep practicing!")
                                .font(.caption)
                        ...
                        if let next = ExerciseLibrary.next(after: exercise.id) {
                            NavigationLink {
                                ExerciseView(
                                    ...
                                    pitchDetector: AudioPitchDetector()
                                )
                            } label: {
...
private struct ConfettiCanvas: View {
    ...
    struct Particle {
        ...
        static func generate(seed: Int) -> Particle {
            ...
            return Particle(
                x: Double(seed * 37 % 100) / 100.0,
                angle: Double(seed * 13 % 360) * .pi / 180,
                speed: 0.15 + Double(seed * 7 % 30) / 100.0,
                color: colors[seed % colors.count],
                size: 6 + Double(seed % 8)
            )
        }
    }
    ...
    var body: some View {
        Canvas { ctx, size in
            for p in particles {
                ...
                ctx.fill(Path(rect), with: .color(p.color.opacity(alpha)))
            }
        }
    }
}
