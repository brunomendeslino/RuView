...
struct RandomChordView: View {
                ...
                ScrollView {
                    VStack(spacing: CT.largeSpacing) {
                        ...
                        VStack(spacing: CT.smallSpacing) {
                            HStack(spacing: CT.smallSpacing) {
                                Button {
                                    Task { await synth.playChord(currentChord, duration: 2.0) }
                                } label: {
                                    Label("Chord", systemImage: "play.circle.fill")
                                ...
                                Button {
                                    Task { await synth.arpeggiate(currentChord, noteDuration: 1.0) }
                                } label: {
                                    Label("Notes", systemImage: "play.square.stack")
                                        .frame(maxWidth: .infinity)
                                }
                                .buttonStyle(GlassButtonStyle())
                                .disabled(synth.isPlaying)
                            ...
                            Button { newChord() } label: {
                                Label("New Chord", systemImage: "shuffle")
                                    .frame(maxWidth: .infinity)
                            }
                            .buttonStyle(GlassButtonStyle())
                            .disabled(synth.isPlaying)
                        ...
                        Button {
                            navigateToExercise = buildExercise()
                        } label: {
    ...
    private func newChord() {
        if let type = filterType {
            currentChord = ChordLibrary.random(type: type)
        } else {
            currentChord = ChordLibrary.random()
        }
        isReady = false
    }
