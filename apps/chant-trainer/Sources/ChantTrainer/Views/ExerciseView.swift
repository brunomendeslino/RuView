...
struct ExerciseView: View {
    ...
    var body: some View {
            ...
            VStack(spacing: CT.largeSpacing) {
                HStack {
                    VStack(alignment: .leading, spacing: 2) {
                        ...
                        Text(exercise.difficulty.displayName).font(.caption).foregroundStyle(exercise.difficulty.color)
                    }
                    Spacer()
                    Text("XP: \(totalXPEarned)").font(.caption.monospacedDigit()).foregroundStyle(.secondary)
                    Button { dismiss() } label: {
                        Image(systemName: "xmark").font(.caption).padding(CT.smallSpacing).glassCard()
                    }.foregroundStyle(.primary)
                }
                .padding(.horizontal, CT.largeSpacing)

                if case .active(let idx, _) = phase { progressDots(currentIndex: idx) }
                    ...
                    case .chordPlayback, .waitingForReady:
                        if let chord = exercise.chord {
                            ChordDeconstructionView(chord: chord, isReady: $chordReady, synth: synth)
                                .onChange(of: chordReady) { _, ready in if ready { startNotePhase(index: 0) } }
                        }
                    case .active(let idx, let elapsed): activeView(noteIndex: idx, elapsed: elapsed)
                    case .noteResult(let grade, _): noteResultView(grade: grade)
                    case .finished: Color.clear
                    }
                }
                .padding(.horizontal, CT.largeSpacing)
            }
        ...
        .fullScreenCover(isPresented: Binding(
            ...
            ResultsView(exercise: exercise, stars: computeStars(), xpEarned: totalXPEarned,
                accuracy: computeAccuracy(), newAchievements: newAchievements,
                progressModel: progress, onHome: { dismiss() })
        }
    }
    ...
    private func countdownView(_ n: Int) -> some View {
        VStack(spacing: CT.largeSpacing) {
            Spacer()
            Text("\(n)").font(.system(size: 120, weight: .black)).foregroundStyle(.primary)
                .transition(.scale.combined(with: .opacity)).id("countdown_\(n)")
            Text("Get ready…").font(.title3).foregroundStyle(.secondary)
            Spacer()
        ...
        return Group {
            if isCompact {
                VStack(spacing: CT.spacing) {
                    NoteDisplayView(targetNote: target.note, centsOff: pitchDetector.currentCentsOff, isDetecting: isDetecting)
                    timingBar(fraction: fraction)
                    PitchMeterView(centsOff: pitchDetector.currentCentsOff, isActive: isDetecting)
                }
            } else {
                VStack(spacing: CT.spacing) {
                    HStack(spacing: CT.largeSpacing) {
                        NoteDisplayView(targetNote: target.note, centsOff: pitchDetector.currentCentsOff, isDetecting: isDetecting).frame(maxWidth: .infinity)
                        PitchMeterView(centsOff: pitchDetector.currentCentsOff, isActive: isDetecting).frame(maxWidth: .infinity)
                    }
                    timingBar(fraction: fraction)
                }
            }
    ...
    private func noteResultView(grade: NoteGrade) -> some View {
        VStack(spacing: CT.spacing) {
            ...
            Image(systemName: grade == .miss ? "xmark.circle" : "checkmark.circle.fill")
                .font(.system(size: 64)).foregroundStyle(grade.color)
            Text(grade.label).font(.largeTitle.bold()).foregroundStyle(grade.color)
            Text("+\(grade.rawValue) XP").font(.title3).foregroundStyle(.secondary)
            Spacer()
        }.frame(maxWidth: .infinity)
    }
    ...
    private func progressDots(currentIndex: Int) -> some View {
        HStack(spacing: 8) {
            ForEach(0..<exercise.targets.count, id: \.self) { i in
                Circle().fill(i < currentIndex ? Color.green : i == currentIndex ? Color.accentColor : Color.primary.opacity(0.3))
    ...
    private func startCountdown() {
        ...
        countdownTimer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { _ in
            n -= 1
            if n > 0 { withAnimation { phase = .countdown(n) } }
            else {
                ...
                if exercise.mode == .mimicChord && exercise.chord != nil {
                    ...
                    Task {
                        if let firstNote = exercise.targets.first?.note { await synth.play(note: firstNote, duration: 1.5) }
                        startNotePhase(index: 0)
                    }
                } else { startNotePhase(index: 0) }
            }
        }
    }

    private func startNotePhase(index: Int) {
        guard index < exercise.targets.count else { finishExercise(); return }
        centsReadings = []
        let target = exercise.targets[index]
        noteDuration = target.durationBeats; noteElapsed = 0
        withAnimation { phase = .active(noteIndex: index, elapsed: 0) }
        if !pitchDetector.isRunning { Task { try? pitchDetector.startCapture() } }
        ...
        noteTimer = Timer.scheduledTimer(withTimeInterval: 0.05, repeats: true) { _ in
            ...
            if noteElapsed >= target.durationBeats { noteTimer?.invalidate(); noteTimer = nil; evaluateNote(index: index) }
        }
    }

    private func evaluateNote(index: Int) {
        let grade: NoteGrade = centsReadings.isEmpty ? .miss :
            NoteGrade.from(centsOff: centsReadings.reduce(0, +) / Double(centsReadings.count))
        noteGrades.append(grade); totalXPEarned += grade.rawValue
        let next = index + 1 < exercise.targets.count ? index + 1 : nil
        withAnimation { phase = .noteResult(grade: grade, nextIndex: next) }
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
            if let nextIndex = next { startNotePhase(index: nextIndex) } else { finishExercise() }
        }
    }

    private func finishExercise() {
        pitchDetector.stopCapture()
        let acc = computeAccuracy(); let stars = computeStars()
        totalXPEarned += stars == 3 ? 20 : 0
        let beforeAchievements = progress.achievements
        progress.recordExerciseComplete(id: exercise.id, stars: stars, xpEarned: totalXPEarned,
            accuracy: acc, chordName: exercise.chord?.displayName)
    ...
    private func computeAccuracy() -> Double {
        ...
        return Double(noteGrades.filter { $0 != .miss }.count) / Double(noteGrades.count)
    }

    private func computeStars() -> Int {
        let acc = computeAccuracy()
        if acc >= 0.9 { return 3 }; if acc >= 0.7 { return 2 }; if acc >= 0.5 { return 1 }; return 0
    }

    private func cleanUp() {
        countdownTimer?.invalidate(); noteTimer?.invalidate()
        pitchDetector.stopCapture(); synth.stop()
    }
}
